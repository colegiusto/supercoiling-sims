function dxdt = ODE(~,x)
    ...
    N = 5; % number of genes in the network
    ...
    a_sigma = 0.1; %rate of positive supercoiling from upstream genes
    ...
    k = 0.1; %Gene network edge strength

    sigma_star = [0 0 -1 -1 0]; % optimal supercoiling densities
    ...
    dxdt(10) = alpha(4)*k_sigma(x(12), sigma_star(4))*(k*x(8)/(1+k*x(8)))-dm(4)*x(10);  
    dxdt(13) = alpha(5)*k_sigma(x(15), sigma_star(5))*(k*x(10))/(1+k*x(10))- dm(5)*x(13);%mRNA for cspA
    
    for i = 1:N
        j = 3*(i-1);
        dxdt(j+2) = a_p*x(j+1)-dp*x(j+2);
        dxdt(j+3) = -5*(x(2)+x(5))*x(j+3)-(dxdt(j+1)-dm(i)*x(j+1))*1+a_sigma;
    end
